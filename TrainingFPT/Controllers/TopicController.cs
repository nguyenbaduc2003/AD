...
namespace TrainingFPT.Controllers
{
    public class TopicController : BaseController<TopicController>
    {
        ...
        public IActionResult Index(string SearchString)
        {
            ...
            {
                model = model.Where(x => (x.CourseName != null ? x.CourseName.Contains(SearchString) : true) || x.NameTopic.Contains(SearchString) || (!string.IsNullOrEmpty(x.Description) ? x.Description.Contains(SearchString) : true)).ToList();
            }
            return View(model);
        }
        [HttpGet]
